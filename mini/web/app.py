...
def hide():
    ...
    if method == "dct":
        ...
        arr = np.stack([np.clip(Y2,0,255).astype(np.uint8), np.clip(Cb2,0,255).astype(np.uint8), np.clip(Cr2,0,255).astype(np.uint8)], axis=2)
...
def extract():
    ...
    try:
        ...
        try:
            ...
            header_reps = 8
            candidates = [2.0, 1.5, 2.5, 3.0, 3.5, 4.0]
            plaintext = None
            ...
            if plaintext is None:
                try:
                    ...
                    buf2 = io.BytesIO()
                    img.save(buf2, format="JPEG", quality=92)
                    buf2.seek(0)
                    Y, Cb, Cr = _img_to_ycbcr_arrays(buf2.getvalue())
                    for delta in candidates:
                        try:
                            raw_header_bits = extract_bits(Y, Cb, Cr, bit_count=32 * header_reps, delta=delta)
                            voted_header_bits = []
