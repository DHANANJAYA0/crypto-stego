...
from .adaptive_embed import rank_blocks_by_variance, choose_channel_for_block
...
def embed_bits(Y: np.ndarray, Cb: np.ndarray, Cr: np.ndarray, bits: List[int], delta: float = 2.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Embed bits into adaptively selected blocks/channels.

    Returns updated Y, Cb, Cr. Pads channels to multiples of 8 internally, then crops back.
    """
    if not bits:
        return Y, Cb, Cr
    ...
    ranked = rank_blocks_by_variance(Y_pad, block_size=block_size)

    # Convert to block maps for quick access
    ...
    for (bi, bj), _score in ranked:
        if not bits:
            break
        Y_blk = Y_blocks[(bi, bj)]
        coeffs = dct2(Y_blk)
        coeffs, bits = _embed_bits_in_coeffs(coeffs, mask, bits, delta)
        Y_blocks[(bi, bj)] = idct2(coeffs)
...
def extract_bits(Y: np.ndarray, Cb: np.ndarray, Cr: np.ndarray, bit_count: int, delta: float = 2.0) -> List[int]:
    """Extract `bit_count` bits using the same adaptive selection, in the same order."""
    if bit_count <= 0:
        return []
    ...
    for (bi, bj), _score in ranked:
        ...
        Y_blk = Y_blocks[(bi, bj)]
        coeffs = dct2(Y_blk)
        bits.extend(_decode_bits_from_coeffs(coeffs, mask, bit_count - len(bits), delta))
